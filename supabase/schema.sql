...
create policy "Owners can update places" on public.places for update to authenticated using (auth.uid() = created_by) with check (auth.uid() = created_by);
create policy "Owners can delete places" on public.places for delete to authenticated using (auth.uid() = created_by);
...
create policy "Admins can manage all places" on public.places for all to authenticated using (public.is_admin()) with check (public.is_admin());
